...
public class claseC {
	...
	public claseC(String ip, int opchost, int host) {
...
*/
		...
		double subredess;
			...
			bits_ocupados=8-bits_hxs;
			double subredess;
			subredess=Math.pow(2, 8-bits_hxs);
			subredes=(int)subredess;
		}
if (subredes>256) {
	ErrorIP=true;
...
*/	
		if(opc==2) {
			...
			bits_hxs--;
			hostxsubred=(int)hxs;
			if (bits_hxs>8) { //Si se ocuparon mas de 24 bits, entonces podemos decir que la cantidad de subredes es imposible.
				ErrorIP=true;
			...
			double hxs;
			hxs=Math.pow(2,8-bits_ocupados);
			hostxsubred=(int)hxs;
		}
		if (hostxsubred>256) {
			ErrorIP=true;
	...
	public String calcular_nuevaMascara() {
		int moct1=255, moct2=255, moct3=255, moct4=0;
		double aux=0;
		int i=7, vueltas=0;
